...
| `55` | Local model check | against a stub endpoint: a structured tool call is `yes`, a call written as text is `no`, a failed request and an unreachable endpoint are `UNKNOWN`; exit 0 only when `KIT_LOCAL_MODEL` is set and passed |
