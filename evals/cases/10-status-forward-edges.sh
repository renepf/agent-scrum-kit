...
sandbox_ticket 1 backlog

ok=0; bad=""
for step in "planned:product-owner" "in-progress:engineer-a" "rfr:engineer-a" \
            "in-review:qa-ruthless" "rft:qa-ruthless" "in-testing:acceptance-tester" "done:product-owner"; do
  st="${step%%:*}"; who="${step##*:}"
...
observe "$ok/7 Kanten akzeptiert${bad:+, abgelehnt:$bad}"
